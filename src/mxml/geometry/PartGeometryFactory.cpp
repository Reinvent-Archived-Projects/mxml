...
std::unique_ptr<PartGeometry> PartGeometryFactory::build() {
    _partGeometry.reset(new PartGeometry(_part, _scoreProperties, _metrics, _spans));
...
void PartGeometryFactory::placeDirection(PlacementGeometry& geometry) {
    ...
    if (geometry.placement() == dom::kPlacementAbove) {
        location.y = _partGeometry->staffOrigin(geometry.staff()) - Metrics::kStaffLineSpacing;
        geometry.setVerticalAnchorPointValues(1, 0);
    } else {
        location.y = _partGeometry->staffOrigin(geometry.staff()) + Metrics::staffHeight() + Metrics::kStaffLineSpacing;
...
void PartGeometryFactory::buildWedge(const MeasureGeometry& startMeasureGeom, const dom::Direction& startDirection,
    ...
    if (placement == dom::kPlacementAbove) {
        startLocation.y = stopLocation.y = _partGeometry->staffOrigin(staff) - _partGeometry->staffDistance()/2;
    } else if (placement == dom::kPlacementBelow) {
        startLocation.y = stopLocation.y = _partGeometry->staffOrigin(staff) + Metrics::staffHeight() + _partGeometry->staffDistance()/2;
    }
...
void PartGeometryFactory::buildPedal(const MeasureGeometry& startMeasureGeom, const dom::Direction& startDirection,
    ...
    int staff = startDirection.staff();
    startLocation.y = stopLocation.y = _partGeometry->staffOrigin(staff) + Metrics::staffHeight() + _partGeometry->staffDistance()/2 - _partGeometry->stavesHeight()/2;
